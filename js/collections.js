...
function applyFilters() {
  ...
  filteredItems = allItems.filter((item) => {
    ...
    // 2) 카테고리(checkbox) 필터
    const matchCategory =
      checkedValues.length === 0 || checkedValues.includes(item.category);
...
card.innerHTML = `
  <div class="thumb-frame">
    <a href="collection_detail.html?id=${item.id}" class="item-link">
...
function renderPagination() {
  ...
  for (let p = 1; p <= totalPages; p++) {
    pagination.appendChild(
      createLink(String(p), p, false, p === currentPage)
    );
  }
